...
class PointEmitter:
    def __init__(self, pos, max_particles = 10, emit_speed = 1, direction = 0, rot_vel = 0, rot_vel_rand = 0, spread = 360, vel = 10, vel_rand = 0, image = "square.png", size = 1, size_rand = 0, drag = 1, lifetime = 1, lifetime_rand = 0, batch = None, group = None):
        ...
        # particle parameters
        self._particle_rot_vel = rot_vel
        self._particle_rot_vel_rand = rot_vel_rand
        self._particle_vel = vel
        self._particle_vel_rand = vel_rand
        self._particle_size = size
        self._particle_size_rand = size_rand
        self._particle_drag = drag
        self._particle_image = image
        self._particle_lifetime = lifetime
        self._particle_lifetime_rand = lifetime_rand
    ...
    def update(self, dt):
        ...
        if self._time_since_emit > 1 / self._emit_speed and len(self._particles) < self._max_particles:
    ...
    def __emit__(self):
        ...
        rot_vel = self._particle_rot_vel
        if self._particle_rot_vel_rand != 0: rot_vel += randrange(-self._particle_rot_vel_rand // 2, self._particle_rot_vel_rand // 2)

        vel = self._particle_vel
    ...
    def set_pos(self, x, y, direction = None):
        ...
        if self._debug:
            self._debug_vertex_list[0].vertices = [self.x, self.y,
                                                   self.x + (cos((self._direction - (self._spread // 2)) * pi / 180) * self._particle_vel * self._particle_lifetime),
                                                   self.y + (sin((self._direction - (self._spread // 2)) * pi / 180) * -self._particle_vel * self._particle_lifetime)]
            self._debug_vertex_list[1].vertices = [self.x, self.y,
                                                   self.x + (cos((self._direction + (self._spread // 2)) * pi / 180) * self._particle_vel * self._particle_lifetime),
                                                   self.y + (sin((self._direction + (self._spread // 2)) * pi / 180) * -self._particle_vel * self._particle_lifetime)]
            self._debug_vertex_list[2].vertices = [self.x, self.y,
                                                   self.x + (cos(self._direction * pi / 180) * self._particle_vel * self._particle_lifetime),
    ...
    def set_intensity(self, vel = None, vel_rand = None, rot_vel = None, rot_vel_rand = None, emit_speed = None, size = None, size_rand = None, spread = None, max_particles = None, lifetime = None, lifetime_rand = None):
        ...
        if spread is not None: self._spread = spread

        # particle parameters
        if vel is not None: self._particle_vel = vel
        if rot_vel is not None: self._particle_rot_vel = rot_vel
        if rot_vel_rand is not None: self._particle_rot_vel_rand = rot_vel_rand
        if vel_rand is not None: self._particle_vel_rand = vel_rand
        if size is not None: self._particle_size = size
        if size_rand is not None: self._particle_size_rand = size_rand
        if lifetime is not None: self._particle_lifetime = lifetime
    ...
    def debug_enable(self, batch, group = None):
        ...
        angle1 = batch.add(2, pgl.gl.GL_LINES, group, ('v2f/stream', (
            self.x, self.y, self.x + (cos((self._direction - (self._spread // 2)) * pi / 180) * self._particle_vel * self._particle_lifetime),
            self.y + (sin((self._direction - (self._spread // 2)) * pi / 180) * -self._particle_vel * self._particle_lifetime))), ('c3B/static', (50, 50, 255, 50, 50, 200)))
        ...
        direction = batch.add(2, pgl.gl.GL_LINES, group, ('v2f/stream', (
            self.x, self.y, self.x + (cos(self._direction * pi / 180) * self._particle_vel * self._particle_lifetime),
            self.y + (sin(self._direction * pi / 180) * -self._particle_vel * self._particle_lifetime))), ('c3B/static', (255, 0, 0, 200, 0, 0)))
